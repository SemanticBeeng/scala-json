...
object ComplexCreature {
  ...
  def notEqualReads[T](v: T)(implicit r: Reads[T]): Reads[T] =
    Reads.filterNot(ValidationError("validate.error.unexpected.value", v))(_ == v)
  ...
  implicit val complexCreatureReads: Reads[ComplexCreature] = (
    ...
    (__ \ "favorites").read(
      ...
      (__ \ "friends").lazyRead(Reads.list[ComplexCreature](complexCreatureReads)) and
      (__ \ "phoneNumbers").read(Reads.list[PhoneNumber](Json.reads[PhoneNumber])) and
      (__ \ "social").readOpt[String] //@todo readOpt??
